  ...
  return (
    ...
    >
      <div>
        {sidebarClose ? (
          <>
            <NewMessage isSidebarClose={true}/>
            {sidebarItems.map((item, index) => (
              <div
                key={index}
                className={`p-4 cursor-s hover:bg-gray-200 ${
                  isCurrentPath(item.path) && "bg-gray-200"
                } rounded-t-3xl`}
              >
                <Link href={item.path}>
          ...
          <>
            <NewMessage isSidebarClose={false}/>

            {sidebarItems.map((item, index) => (
              <Link key={index} href={item.path}>
                <div
                  className={`flex items-center p-4 cursor-pointer rounded-lg hover:bg-gray-200 ${
                    isCurrentPath(item.path) ? "bg-gray-200" : ""
                  } my-2`}
                >
